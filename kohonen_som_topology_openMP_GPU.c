/**
 * KOHONEN SOM - VERSÃO OPENMP PARA GPU (TARGET OFFLOADING)
 *
 ...
 *
 * COMPILAÇÃO (GPU OFFLOAD):
 * gcc -fopenmp -foffload=nvptx-none -O3 kohonen_som_topology_openMP_GPU.c -o som_gpu -lm
 *
 * EXECUÇÃO:
 * ./som_gpu banking [num_threads]
 * ./som_gpu benchmark
 *
 ...
 *
 * MUDANÇAS PARA GPU:
 * - substituição dos loops internos por offload via #pragma omp target
 * - uso de map(to:...), map(from:...), map(tofrom:...)
 * - minimização de transferências entre host e device
 * - uso de reduction(min:...) para BMU
 * - proteção de escrita em pesos via atomic no device
 */
...
#define BENCHMARK_SAMPLE_LIMIT 12000
#define BENCHMARK_ALPHA_MIN 0.4
                              ...
                              double alpha, int R)
{
    ...
    int total_weights = W->dim1 * W->dim2 * W->dim3;
    int stride_x = W->dim2 * W->dim3;
    int stride_y = W->dim3;
    double *weight_data = W->data;
    double *D_buffer = (D && num_out > 0) ? D[0] : NULL;
    int grid_size = num_out * num_out;
    double d_min_candidate = INFINITY;

#if defined(_OPENMP)
    if (D_buffer)
    {
        // [GPU-OFFLOAD] Este loop foi movido para GPU usando target teams distribute parallel for
#pragma omp target teams distribute parallel for collapse(2) \
    map(to:X[0:num_features], weight_data[0:total_weights]) \
    map(from:D_buffer[0:grid_size]) reduction(min : d_min_candidate)
        for (x = 0; x < num_out; x++)
        {
            for (y = 0; y < num_out; y++)
            {
                ...
                {
                    int idx = x * stride_x + y * stride_y + k;
                    double diff = weight_data[idx] - X[k];
                    dist += diff * diff;
                }
                double value = sqrt(dist);
                D_buffer[x * num_out + y] = value;
                if (value < d_min_candidate)
                    d_min_candidate = value;
            }
        }
    }
    else
#endif
    {
        for (x = 0; x < num_out; x++)
        {
            for (y = 0; y < num_out; y++)
            {
                ...
                D[x][y] = sqrt(D[x][y]);
                if (D[x][y] < d_min_candidate)
                    d_min_candidate = D[x][y];
            }
        }
    }
    ...
    if (d_min_candidate < d_min)
        d_min = d_min_candidate;
    ...
    // [GPU-OFFLOAD] Este loop foi movido para GPU usando target teams distribute parallel for
#pragma omp target teams distribute parallel for collapse(2) \
    map(to:X[0:num_features]) map(tofrom:weight_data[0:total_weights])
    for (x = from_x; x < to_x; x++)
    {
        for (y = from_y; y < to_y; y++)
        {
            ...
            {
                int idx = x * stride_x + y * stride_y + k;
                double delta = alpha * scale_factor * (X[k] - weight_data[idx]);
#pragma omp atomic update
                weight_data[idx] += delta;
            }
        }
    }
#else
...
#pragma omp parallel
        {
            ...
            double *D_local_data =
                (double *)malloc(num_out * num_out * sizeof(double));
            for (int i = 0; i < num_out; i++)
                D_local[i] = D_local_data + i * num_out;
...
#pragma omp critical
        ...
        double *D_data = (double *)malloc(num_out * num_out * sizeof(double));
        for (int i = 0; i < num_out; i++)
            D[i] = D_data + i * num_out;
...
#pragma omp critical
    ...
    int is_contiguous = 0;
    if (num_samples > 0 && X && X[0])
    {
        is_contiguous = 1;
        for (int i = 1; i < num_samples; i++)
        {
            if (X[i] != X[0] + i * num_features)
            {
                is_contiguous = 0;
                break;
            }
    ...
    double (*X_matrix)[num_features] = NULL;
    if (num_samples > 0)
    {
        if (is_contiguous)
            X_matrix = (double (*)[num_features])X[0];
        else
        {
            temp_buffer =
                (double *)malloc(num_samples * num_features * sizeof(double));
            for (int i = 0; i < num_samples; i++)
            {
                for (int j = 0; j < num_features; j++)
                    temp_buffer[i * num_features + j] = X[i][j];
            }
            X_matrix = (double (*)[num_features])temp_buffer;
        }
    }
    ...
    if (X_matrix)
    {
        // [GPU-OFFLOAD] Este loop foi movido para GPU usando target teams distribute parallel for
#pragma omp target teams distribute parallel for collapse(2) \
    map(to:min_vals[0:num_features]) map(to:max_vals[0:num_features]) \
    map(tofrom:X_matrix[0:num_samples][0:num_features])
        for (int i = 0; i < num_samples; i++)
        {
            for (int j = 0; j < num_features; j++)
            {
                ...
                if (range > 1e-10)
                    X_matrix[i][j] = (X_matrix[i][j] - min_vals[j]) / range;
                else
                    X_matrix[i][j] = 0.0;
            }
        }
 ...
 * \param[in] max_samples limite de linhas (<=0 para usar todas)
...
double **load_banking_data(const char *filename, int *num_samples, int *num_features,
                           int max_samples)
 {
    ...
    rewind(fp);
    if (fgets(header, sizeof(header), fp) == NULL)  // pular cabeçalho novamente
    {
        fclose(fp);
        return NULL;
    }
    ...
    *num_features = 8;  // age, balance, duration, campaign, previous, default, housing, loan
    int total_samples = line_count;
    int target_samples = (max_samples > 0 && max_samples < total_samples)
                             ? max_samples
                             : total_samples;
    *num_samples = target_samples;

    // Alocar matriz
    double **X = (double **)malloc(target_samples * sizeof(double *));
    for (int i = 0; i < target_samples; i++)
    {
     ...
     // Ler dados
     int sample_idx = 0;
    while (fgets(buffer, sizeof(buffer), fp) && sample_idx < target_samples)
     {
    ...
    *num_samples = sample_idx;  // ajustar número real de amostras
    ...
    if (max_samples > 0 && target_samples < total_samples)
    {
        printf("Carregados %d/%d amostras (limite benchmark de %d)\n",
               *num_samples, total_samples, target_samples);
    }
    else
    {
  ...
  */
void test_banking(int benchmark_mode)
{
#ifdef _OPENMP
    ...
    int num_out = 30;
    const int dataset_limit = benchmark_mode ? BENCHMARK_SAMPLE_LIMIT : -1;
    const double alpha_target = benchmark_mode ? BENCHMARK_ALPHA_MIN : 1e-4;
    ...
    if (benchmark_mode)
    {
        printf("  >> Modo benchmark ativo: limitando dados para %d amostras\n",
               BENCHMARK_SAMPLE_LIMIT);
        printf("  >> Iteracoes reduzidas: alpha minimo = %.2f (~%.0f iteracoes)\n\n",
               BENCHMARK_ALPHA_MIN, (1.0 - BENCHMARK_ALPHA_MIN) / 0.001);
    }
    ...
    kohonen_som(X, &W, num_samples, num_features, num_out, alpha_target);
...
void benchmark_openmp()
{
    ...
    printf("==================================================\n");
    printf("  Dataset benchmark: %d amostras (subset de 45211)          \n",
           BENCHMARK_SAMPLE_LIMIT);
    printf("  Features: 8 selecionadas + normalizacao min-max           \n");
    printf("  Topologia: SOM 30x30 (900 neuronios)                      \n");
    printf("  Iteracoes aprox.: ~%.0f (alpha minimo %.2f)                \n",
           (1.0 - BENCHMARK_ALPHA_MIN) / 0.001, BENCHMARK_ALPHA_MIN);
